...
function Vahimanelist() {
  ...
  const [editUser, setEditUser] = useState<UsersInterface | null>(null);// @ts-ignore
  ...
  const getUsers = async () => {
    try {
      ...
      if (res && Array.isArray(res)) {
        const adminUsers = res.filter((user: UsersInterface) => user.UserRoleID === 6);
        setUsers(adminUsers);
      } else {
  ...
  return (
    <>
      ...
      <Row>
        <Col span={12}>
          <h2 style={{ fontSize: "28px", fontWeight: "bold", color: "#B25900" }}>Manage VahicleManegers</h2>
        </Col>
      </Row>
      <Divider />
      <Table
